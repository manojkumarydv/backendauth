  ...
  static userRegistration = async (req, res) => {
    ...
    if (name && email && password && tc) {
      try {
        ...
        return res.status(201).send({ "status": "success", "message": "Registration Success", "token": token,"isAdmin": saved_user.isAdmin });
      } catch (error) {
        console.log(error);
      ...
      if (email && password) {
        ...
        if (user != null) {
          ...
          if ((user.email === email) && isMatch) {
            ...
            res.send({ "status": "success", "message": "Login Success", "token": token ,"isAdmin": user.isAdmin})
          } else {
